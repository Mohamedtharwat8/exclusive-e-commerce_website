...
const Cartlist = (arr: {
  arr: { title: string; image: string; price: number }[];
}) => {
  return (
    <>
      {arr.arr.map(
        (data: { title: string; image: string; price: number }, index) => {
          return (
            <div
              className="grid grid-cols-4 w-full sm:py-7 py-4 relative group"
              key={index}
            >
              ...
              <div className="col-span-1 m-auto xs:text-base text-sm">
                ${data.price}
              </div>
